...
try:
    ...
    pending_requests_col = db['pending_join_requests']  # NEW: Track join requests
...
👇 Use /start to access the Mini App""",
    ...
    # 🌟 PREMIUM FORCE JOIN MESSAGES
    'force_join_start': """🔐 **ভিডিও আনলক করতে চ্যানেল জয়েন করুন!**

**{name}**, আপনার ভিডিও প্রায় তৈরি! 🎬

নিচের চ্যানেল{plural} জয়েন করুন:

{channels_list}

💡 জয়েন করার পর "Verify" click করুন!""",

    'channel_joined': """✅ **চ্যানেল জয়েন সফল!**

**{channel_name}** ✓

{remaining_text}""",

    'request_sent': """📤 **Request পাঠানো হয়েছে!**

**{channel_name}** ⏳

{remaining_text}""",

    'still_pending': """⏭️ **আরও {count} টি বাকি!**

{channels_list}

👇 জয়েন করে Verify করুন!""",

    'all_verified': """🎉 **সব ভেরিফাই সম্পন্ন!**

🎬 ভিডিও পাঠানো হচ্ছে...""",

    'video_unlocked': """🎊 **ভিডিও আনলক!**
...
async def handle_chat_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    This is the KEY to detecting pending join requests!
    """
    chat_join_request = update.chat_join_request
    user = chat_join_request.from_user
    chat = chat_join_request.chat
    
    logger.info(f"🔔 JOIN REQUEST: User {user.id} (@{user.username}) requested to join {chat.title} (ID: {chat.id})")
    
    # Mark this join request in database
    mark_join_request_sent(user.id, chat.id)
    
    # Optional: Send a message to user confirming we detected their request
    try:
        await context.bot.send_message(
            chat_id=user.id,
            text=f"✅ **Join Request Detected!**\n\n"
                 f"আপনার join request পাঠানো হয়েছে! 🎉\n"
                 f"Your join request has been sent! 🎉\n\n"
                 f"এখন \"✅ আমি জয়েন করেছি\" বাটন ক্লিক করুন!\n"
                 f"Now click \"✅ I Joined\" button!",
            parse_mode=ParseMode.MARKDOWN
        )
    except Exception as e:
        logger.debug(f"Could not send confirmation to user: {e}")
...
# ===================== PREMIUM FORCE JOIN LOGIC =====================

async def check_force_join_progressive(user_id: int, context: ContextTypes.DEFAULT_TYPE, user_name: str = "User") -> dict:
    """🌟 PREMIUM: Progressive force join checker - ALWAYS checks fresh, no cache!"""
    force_channels = get_force_join_channels()
    
    if not force_channels:
        return {'all_joined': True, 'not_joined': [], 'keyboard': None, 'message': None}
    ...
    for channel in force_channels:
        ...
        try:
            if is_private:
                # For private: Check if request sent in last 24 hours (fresh check)
                cutoff_time = datetime.now() - timedelta(hours=24)
                
                pending = pending_requests_col.find_one({
                    'user_id': user_id,
                    'channel_id': channel_id,
                    'requested_at': {'$gte': cutoff_time}
                })
                
                if not pending:
                    not_joined.append(channel)
                else:
                    logger.info(f"✅ User {user_id} has recent request to private {channel_id}")
            else:
                # For public: ALWAYS check via Telegram API (no cache!)
                try:
                    member = await context.bot.get_chat_member(channel_id, user_id)
                    if member.status in ['left', 'kicked']:
                        not_joined.append(channel)
                        logger.info(f"❌ User {user_id} left public channel {channel_id}")
                    else:
                        logger.info(f"✅ User {user_id} is member of public {channel_id}")
                except BadRequest as e:
                    logger.warning(f"⚠️ Cannot check {channel_id}: {e}")
                    not_joined.append(channel)
                except Exception as e:
    ...
    plural = "গুলো" if len(not_joined) > 1 else ""
    
    message = get_message('force_join_start').format(
        name=user_name,
        plural=plural,
        channels_list=channels_list
    )
    
    # Create buttons - support both username and invite link
    buttons = []
    for channel in not_joined:
        ...
        # Prefer username, fallback to invite link
        if username:
            url = f"https://t.me/{username}"
            buttons.append([InlineKeyboardButton(f"{icon} {name}", url=url)])
        elif invite_link:
            buttons.append([InlineKeyboardButton(f"{icon} {name}", url=invite_link)])
        else:
            # No way to join - skip button (admin error)
            logger.error(f"❌ Channel {name} has no username or invite link!")
    
    # Only add verify if we have at least one button
    if buttons:
        buttons.append([InlineKeyboardButton("✅ Verify করুন", callback_data="verify_progressive")])
...
async def verify_progressive(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, user_name: str = "User") -> dict:
    """🌟 PREMIUM: Progressive verification - FRESH check every time, NO cache!"""
    force_channels = get_force_join_channels()
    
    if not force_channels:
        return {'all_done': True, 'message': None, 'keyboard': None}
    ...
    for channel in force_channels:
        ...
        # ❌ REMOVED: No more context.user_data caching!
        # We check FRESH every time so if user leaves, we detect it!
        
        try:
            if is_private:
                # Check if request sent recently
                cutoff_time = datetime.now() - timedelta(hours=24)
                ...
                if pending:
                    newly_joined.append(channel)
                else:
                    still_pending.append(channel)
            else:
                # Public channel - ALWAYS verify via API
                try:
                    member = await context.bot.get_chat_member(channel_id, user_id)
                    ...
                    logger.error(f"Error verifying {channel_id}: {e}")
                    still_pending.append(channel)
        except Exception as e:
            logger.error(f"Error in verify loop for {channel_id}: {e}")
            still_pending.append(channel)
    
    if not still_pending:
        return {'all_done': True, 'message': get_message('all_verified'), 'keyboard': None}
    
    if newly_joined:
        channel = newly_joined[0]
        channel_name = channel.get('name', 'Channel')
        is_private = channel.get('is_private', False)
        emoji = "🔒" if is_private else "📢"
        
        base_msg = get_message('request_sent' if is_private else 'channel_joined')
        
        plural = "গুলো" if len(still_pending) > 1 else ""
        channels_list = format_channels_list(still_pending)
        
        remaining = get_message('still_pending').format(
            count=len(still_pending),
            plural=plural,
            channels_list=channels_list
        )
        
        message = base_msg.format(channel_name=channel_name, emoji=emoji, remaining_text=remaining)
    else:
        plural = "গুলো" if len(still_pending) > 1 else ""
        channels_list = format_channels_list(still_pending)
        
        message = get_message('still_pending').format(
            count=len(still_pending),
            plural=plural,
            channels_list=channels_list
        )
    
    # Create buttons with username OR invite link support
    buttons = []
    for channel in still_pending:
        is_private = channel.get('is_private', False)
        name = channel.get('name', 'Channel')
        username = channel.get('username', '')
        invite_link = channel.get('invite_link', '')
...
async def handle_verify_progressive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """🌟 PREMIUM: Handle verify button"""
    ...
    try:
        ...
        if user_id in user_all_messages:
            messages = user_all_messages[user_id][:]
            for msg_id in messages:
                if msg_id != current_msg_id:
                    try:
                        await context.bot.delete_message(chat_id=user_id, message_id=msg_id)
                        user_all_messages[user_id].remove(msg_id)
                    except:
                        pass
        
        verify_result = await verify_progressive(update, context, user_id, user_name)
        
        if verify_result['all_done']:
            try:
            ...
            if video_id:
                # Create fake update for send_video
                update.callback_query.data = f"verify_{video_id}"
                # Will be handled by existing verify_ handler
                await query.answer("✅ ভেরিফাই সফল!", show_alert=False)
        else:
            try:
                await query.message.edit_text(
                    text=verify_result['message'],
                    reply_markup=verify_result['keyboard'],
                    parse_mode=ParseMode.MARKDOWN
                )
            except:
                try:
                ...
                if user_id not in user_all_messages:
                    user_all_messages[user_id] = []
                user_all_messages[user_id].append(new_msg.message_id)
    
    except Exception as e:
        logger.error(f"❌ Error in verify: {e}")
        await query.answer("❌ ভুল হয়েছে!", show_alert=True)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 🌟 PREMIUM: Progressive verify handler
    if data == "verify_progressive":
        await handle_verify_progressive(update, context)
        return
    ...
    if data.startswith("verify_"):
        ...
        for channel in force_channels:
            try:
                ...
                # Log detailed status
                logger.info(f"🔍 Verify: Checking user {user.id} in {channel['username']}: status={member.status}")
                ...
                if is_member_or_admin:
                    logger.info(f"✅ Verify: User {user.id} IS MEMBER of {channel['username']}")
                    # Clear any pending request
                    clear_join_request(user.id, channel['channel_id'])
                    # Continue to next channel
                elif has_pending_request:
                    logger.info(f"✅ Verify: User {user.id} has PENDING REQUEST in {channel['username']} - GRANTING ACCESS!")
                    # Grant access even though not approved yet!
                    # Continue to next channel
                elif member.status in ['left', 'kicked']:
                    not_joined.append(channel)
                    logger.info(f"❌ Verify: User {user.id} NOT in {channel['username']}: {member.status}")
                else:
                    # Unknown status
                    not_joined.append(channel)
                    logger.warning(f"⚠️ Verify: Unknown status for user {user.id} in {channel['username']}: {member.status}")
...
3. "✅ আমি জয়েন করেছি" ক্লিক করুন"""
            ...
            try:
                ...
                if user.id not in user_all_messages:
                    user_all_messages[user.id] = []
                user_all_messages[user.id].append(new_msg.message_id)
        ...
        # ✅ Clean up all previous messages before sending video
        await cleanup_user_messages(context, user.id, chat_id)
