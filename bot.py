...
import asyncio  # For smooth UX delays
...
async def send_video_direct_by_id(user_id, video_id, context):
    ...
    # Send unlock message
    unlock_text = get_message('video_unlocked').format(
        video_title=video.get('title', 'Video')
    )
    
    unlock_msg = await context.bot.send_message(
        chat_id=user_id,
        text=unlock_text,
        parse_mode=ParseMode.MARKDOWN
    )
    ...
    video_protection = get_setting('video_protection', True)
    ...
    # Send after video message if configured
    after_msg = get_message('after_video')
    after_buttons = get_buttons('after_video')
    
    if after_msg and after_buttons:
        keyboard = []
    ...
    logger.info(f"🎬 Sent video {video_id} to user {user_id}")
